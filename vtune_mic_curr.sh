...
#sem duracao
amplxe-cl -target-duration-type=veryshort -collect $1 -target-system=mic-host-launch:$HOSTNAME-mic$whichcard -r $resultado/$analise --search-dir /opt/mpss/3.3/sysroots/k1om-mpss-linux/boot -- /panfs/panfs3/users1/Xdlcost/raphael/execucao/mic_native.sh" 0 $nome_exec

#para intensidade aritmetica
amplxe-cl -target-duration-type=veryshort -collect-with runsa -knob event-config=VPU_ELEMENTS_ACTIVE,CPU_CLK_UNHALTED -target-system=mic-host-launch:$HOSTNAME-mic$whichcard -r $resultado/$analise --search-dir /opt/mpss/3.3/sysroots/k1om-mpss-linux/boot -- /panfs/panfs3/users1/Xdlcost/raphael/execucao/mic_native.sh" 0 $nome_exec
